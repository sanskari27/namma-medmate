...
export async function patchRole(
  ...
  const { data } = await apiClient.patch<AccessRole>(`${API.ROLES}/${id}`, {
    name,
    modules,
    version,
  });
...
export async function deactivateRole(id: string): Promise<AccessRole> {
  const { data } = await apiClient.post<AccessRole>(`${API.ROLES}/${id}/deactivate`);
  return data;
}
