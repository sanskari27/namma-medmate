...
import { AlertCircle, Eye, EyeOff, Lock, UserRound, WifiOff } from 'lucide-react';
import { CounterPinSignIn } from '@/components/auth/CounterPinSignIn';
...
function tillRole(role: string): string {
    ...
    <div className="grid min-h-screen bg-canvas md:grid-cols-[minmax(0,1fr)_28rem]">
      ...
      <section className="flex min-h-screen flex-col border-l-4 border-brand bg-surface">
        ...
        <Reveal className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center px-6 py-12">
          ...
          {people === null ? (
            <p className="text-sm text-muted">Loading till logins</p>
          ) : null}
          {showPicker ? (
            <div className="space-y-4">
              <div>
                <h1 className="text-xl font-semibold text-ink">Who is at this counter?</h1>
                <p className="mt-1 text-sm text-muted">Tap a saved person and enter their PIN. Faster than typing email.</p>
              </div>
                  ...
                  <li key={person.userId} className="flex items-stretch gap-2">
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        <span className="block truncate font-medium text-ink">{person.displayName}</span>
          ...
          {showPassword ? (
            <form onSubmit={onSubmit} className="space-y-4" noValidate>
              <div>
                ...
                <p className="mt-1 text-sm text-muted">Use the email issued for this counter. After a PIN, this till can remember you.</p>
              </div>
              {listFailed ? (
                <p
                  role="alert"
