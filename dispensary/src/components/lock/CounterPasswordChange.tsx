...
function statusCopy(status: FormStatus): { icon: typeof AlertCircle; text: string } | null {
  switch (status) {
    ...
    case 'denied':
      return { icon: AlertCircle, text: 'Current password does not match this counter. Try again or ask the owner.' };
    case 'conflict':
      return { icon: AlertCircle, text: 'That password was used before on this till. Pick one this counter has not used.' };
    case 'failure':
      return { icon: WifiOff, text: 'Could not reach the server. Save the new password from this counter again.' };
    default:
      return null;
  }
}
