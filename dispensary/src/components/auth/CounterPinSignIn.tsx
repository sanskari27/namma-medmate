...
function statusCopy(status: FormStatus): { icon: typeof AlertCircle; text: string } | null {
  switch (status) {
    ...
    case 'locked':
      return { icon: AlertCircle, text: 'This staff account cannot enter the dispensary. Ask the owner.' };
    case 'conflict':
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Enter the six-digit PIN for this counter. Not the password.</p>
      </div>
      ...
      <div className="grid w-full grid-cols-3 gap-2">
        {KEYS.map((key) => (
          ...
          >
            {key === 'back' ? <Delete className="size-5" aria-hidden="true" /> : key === 'clear' ? 'C' : key}
          </button>
        ))}
      </div>
      ...
      <button type="button" className="text-sm text-brand underline-offset-2 hover:underline" onClick={onBack}>
