...
function statusCopy(status: FormStatus): { icon: typeof AlertCircle; text: string } | null {
  switch (status) {
    ...
    case 'denied':
      return {
        icon: AlertCircle,
        text: 'This counter session expired. Sign in again, then set the PIN.',
      };
    case 'conflict':
      return {
        icon: AlertCircle,
        text: 'This counter already has a PIN. Sign in again with it from the till picker.',
      };
    case 'failure':
      return {
        icon: WifiOff,
        text: 'Could not reach the server. Try saving the PIN from this counter.',
      };
    default:
      return null;
  }
