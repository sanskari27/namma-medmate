...
import { inboxReceived, notificationRead, unreadReceived, type RootState } from '@/store';
import {
  fetchInbox,
  fetchUnreadCount,
  isApiError,
  ...
  const panel = (
    <div className="flex max-h-[min(28rem,calc(100vh-4rem))] flex-col overflow-hidden">
      ...
      {inbox.items.length > 0 && status !== 'loading' && status !== 'empty' ? (
        <ul
          className="panel-scroll min-h-0 flex-1 overflow-x-hidden overflow-y-auto pr-1.5"
          ...
          {inbox.items.map((item) => (
            <li
              ...
              className={cn(
                ...
                item.read ? 'bg-surface' : 'bg-brand-soft/50',
              )}
            >
              <span
              ...
              <div className="min-w-0 flex-1 px-3 py-2.5">
                ...
                {item.body ? (
                  <p className="mt-0.5 break-words text-xs text-muted">{item.body}</p>
                ) : null}
                <p className="mt-1 font-mono text-[11px] text-muted">
                  {floorDestination(item.sourceType)}
                </p>
