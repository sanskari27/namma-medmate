...
function statusCopy(status: FormStatus): { icon: typeof ShieldAlert; text: string } | null {
  switch (status) {
    case 'validation':
      return { icon: ShieldAlert, text: 'Enter the current HQ password and a new secret of at least eight characters.' };
    case 'denied':
      return { icon: ShieldAlert, text: 'Current HQ password was not recognised.' };
    case 'conflict':
      return { icon: ShieldAlert, text: 'That secret is already in this operator history. Choose another.' };
    case 'failure':
    ...
    >
      ...
      >
        ...
        <p className="mt-1 mb-5 text-sm text-muted">
          This operator password expired or was issued as a temporary secret. Set a new one before using the console.
        </p>
        {banner ? (
          <p
            id={statusId}
