...
function statusCopy(status: FormStatus): { icon: typeof ShieldAlert; text: string } | null {
  switch (status) {
    ...
    case 'denied':
      return {
        icon: ShieldAlert,
        text: 'This HQ session is no longer valid. Authenticate again, then set the PIN.',
      };
    case 'conflict':
    ...
    case 'failure':
      return {
        icon: Unplug,
        text: 'The platform API did not save the PIN. Retry from this console.',
      };
    default:
      return null;
  }
    ...
    >
      ...
      >
        ...
        <p className="mt-1 mb-5 text-sm text-muted">
          Idle HQ consoles sign out after five minutes. Choose six digits to authenticate again on
          this console.
        </p>
        {banner ? (
          <p
