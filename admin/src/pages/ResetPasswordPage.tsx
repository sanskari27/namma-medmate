...
function statusCopy(status: FormStatus): { icon: typeof Ban; text: string } | null {
  switch (status) {
    case 'empty':
      return { icon: ShieldAlert, text: 'Open the HQ reset link from email. This console needs that token.' };
    case 'validation':
      return { icon: ShieldAlert, text: 'Use at least eight characters, and confirm the same HQ password.' };
    case 'denied':
      return { icon: Ban, text: 'This HQ reset link is expired or spent. Queue a new MASTER reset.' };
    case 'conflict':
      return { icon: ShieldAlert, text: 'That password is already in this operator history. Choose another.' };
    case 'failure':
      return { icon: Unplug, text: 'The platform API did not save the password. Retry from this console.' };
    case 'success':
      return { icon: ShieldCheck, text: 'HQ password updated. Authenticate on this console with the new secret.' };
    default:
      return null;
  }
}
        ...
        <Reveal className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center px-6 py-12">
          <form onSubmit={onSubmit} className="space-y-4" noValidate>
            <div>
              ...
              <p className="mt-1 text-sm text-muted">Eight or more characters, never reused on this operator.</p>
            </div>
            {banner ? (
              <p
                id={statusId}
