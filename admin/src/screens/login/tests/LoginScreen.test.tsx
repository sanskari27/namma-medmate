...
describe('admin HQ login', () => {
  ...
  it('conflict: stale PIN login is explained', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(
      'This HQ session is stale. Sign in again.',
    );
  });
  ...
  it('success: remove drops that operator from this console', async () => {
    ...
    renderLogin();
    await user.click(
      await screen.findByRole('button', { name: 'Remove Sanskar from this console' }),
    );
    expect(forgetMock).toHaveBeenCalledWith('m1');
    expect(
      await screen.findByRole('button', { name: 'Authenticate Desk ops' }),
    ).toBeInTheDocument();
  ...
  it('locked: 403 blocks the operator account', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Sign in' }));
    expect(await screen.findByRole('alert')).toHaveTextContent(
      'This operator account cannot enter HQ.',
    );
  });
  ...
  it('slider next changes the HQ operation', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Plan and expiry overrides' }),
    ).toBeInTheDocument();
  });
});
