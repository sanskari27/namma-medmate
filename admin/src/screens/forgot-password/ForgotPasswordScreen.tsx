...
function statusCopy(status: FormStatus): { icon: typeof Ban; text: string } | null {
  switch (status) {
    ...
    case 'conflict':
      return {
        icon: ShieldAlert,
        text: 'An HQ reset is already queued. Retry shortly from this console.',
      };
    case 'failure':
      return {
        icon: Unplug,
        text: 'The platform API did not accept the reset. Retry from this console.',
      };
    case 'success':
      return {
        icon: Mail,
        text: 'If this MASTER account can reset by email, a time-limited HQ link was queued.',
      };
    default:
      return null;
  }
          ...
          <form onSubmit={onSubmit} className="space-y-4" noValidate>
            <div>
              ...
              <p className="mt-1 text-sm text-muted">
                MASTER operators receive a time-limited email link. Platform sub-accounts are reset
                from Operator password.
              </p>
            </div>
            {banner ? (
              <p
