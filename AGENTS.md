...
- UI: skill `stitch-ui-design` before React. Compose pharmacy UI into `apps/dispensary-app-web`. Do not create a Platform Admin HQ app unless the user or requirement explicitly starts it.
...
See `.cursor/commands/` (`next-module`, `implement-module`, `design-screens`, `scaffold-module`, `review-module`, `verify-gates`, `add-e2e-screen`, `codegen`, `security-review`, `env-doctor`, `code-review`, `local-up`, `tf-plan`).
...
See `.cursor/skills/`. Primary loop: `implement-module` → `ponytail` → `tdd-enforcer` → `stitch-ui-design` → `verify-gates`.
