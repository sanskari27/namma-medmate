...
class SalesInvoiceRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void invalidCreateLeavesNoPartialInvoiceOrSequence() throws Exception {
    Tenant tenant = persistTenant("roll-inv", "Roll Inv");
    persistPlan(tenant.getId(), PlanCode.FREE);
    persistUser(tenant.getId(), "owner@roll-inv.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId());
    Cookie cookie = login("owner@roll-inv.local");
    mockMvc
        .perform(
    ...
    assertThat(salesInvoiceRepository.count()).isZero();
    assertThat(salesInvoiceLineRepository.count()).isZero();
    assertThat(salesInvoiceSequenceRepository.count()).isZero();
