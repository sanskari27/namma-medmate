...
class HospitalArRollbackTest extends AbstractIntegrationTest {

  private static final String PASSWORD = "counter-pass-1";
  private static final Instant T0 = Instant.parse("2026-09-20T12:30:00Z");

  @SpyBean private InventoryStockService inventoryStockService;
  ...
  @Test
  void restockFailureLeavesWardStockAndArUnchanged() throws Exception {
    Fixture fx = seedPro("ar-roll");
    configure(fx);
                ...
                post("/api/v1/hospital/issues")
                    ...
                    .content(
                        """
                        {
                          "wardId":"%s",
                          "reason":"FLOOR_STOCK",
                          "idempotencyKey":"ar-roll-iss",
                          "lines":[{"productId":"%s","batchId":"%s","quantity":3}]
                        }
                        """
                            .formatted(wardBed.wardId(), stocked.productId(), stocked.batchId())))
    ...
    UUID issueId =
        UUID.fromString(
            objectMapper
                ...
                .path("id")
                .asText());
    BigDecimal wardBefore = hospitalWardStockRepository.findAll().get(0).getQuantity();
    long arBefore = hospitalCreditAccountRepository.findAll().get(0).getBalancePaise();
    BigDecimal pharmacyBefore =
    ...
    doThrow(new IllegalStateException("stock exploded"))
        .when(inventoryStockService)
        .restockHospitalReturn(any(), any(), any(), any(), any());
        ...
        .perform(
            post("/api/v1/hospital/returns")
                ...
                .content(
                    ...
                    {
                      "issueId":"%s",
                      "idempotencyKey":"ar-roll-ret",
                      "lines":[{"productId":"%s","quantity":1}]
                    }
                    """
                        .formatted(issueId, stocked.productId())))
        .andExpect(status().is5xxServerError());

    assertThat(hospitalReturnRepository.count()).isZero();
    assertThat(hospitalReturnLineRepository.count()).isZero();
    ...
    assertThat(hospitalWardStockRepository.findAll().get(0).getQuantity())
        .isEqualByComparingTo(wardBefore);
    ...
    assertThat(
            stockBalanceRepository
                ...
                .orElseThrow()
                .getQuantity())
        .isEqualByComparingTo(pharmacyBefore);
  }
