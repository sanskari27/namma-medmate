...
class ProductRollbackTest {
  ...
  static final PostgreSQLContainer<?> POSTGRES =
      new PostgreSQLContainer<>("postgres:16-alpine")
          .withDatabaseName("nammamedmate_product_rollback")
  ...
  @Test
  void invalidCreateLeavesNoPartialProductOrCategory() throws Exception {
    Tenant tenant = persistTenant("roll-prod", "Roll Product");
    persistPlan(tenant.getId(), PlanCode.FREE);
    persistUser(tenant.getId(), "owner@roll-prod.local", AppUserRole.pharmacy_owner);
    Cookie cookie = login("owner@roll-prod.local");
    ...
    mockMvc
        .perform(
            post("/api/v1/manufacturers")
                ...
                .content("{\"name\":\"\"}"))
        .andExpect(status().isBadRequest());
    assertThat(manufacturerRepository.count()).isZero();
