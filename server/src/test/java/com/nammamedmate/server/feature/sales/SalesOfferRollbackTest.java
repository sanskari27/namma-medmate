...
class SalesOfferRollbackTest extends AbstractIntegrationTest {
  ...
  @Autowired private SalesOfferProductRepository salesOfferProductRepository;
  @Autowired private SalesInvoiceLineRepository salesInvoiceLineRepository;
  @Autowired private PasswordEncoder passwordEncoder;

  @Test
  void ac05_failedCreateAndApplyLeaveNoPartialOfferOrSnapshot() throws Exception {
    Tenant tenant = persistTenant("offer-roll", "Offer Roll");
    persistPlan(tenant.getId());
    persistUser(tenant.getId(), "owner@offer-roll.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId());
    Cookie cookie = login("owner@offer-roll.local");
    mockMvc
        .perform(
    ...
    mockMvc
        .perform(
            post("/api/v1/inventory/receipts")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"productId\":\"%s\",\"batchNumber\":\"LOT-OR\",\"manufacturedOn\":\"2026-01-15\",\"expiresOn\":\"2027-06-30\",\"purchasePricePaise\":12500,\"quantity\":\"10\",\"idempotencyKey\":\"offer-roll-recv\",\"expectedVersion\":0}"
                        .formatted(productId)))
        .andExpect(status().isOk());
    UUID batchId =
        UUID.fromString(
    ...
    long offersBefore = salesOfferRepository.count();
    long productsBefore = salesOfferProductRepository.count();
    mockMvc
        .perform(
            post("/api/v1/offers")
                .cookie(cookie)
    ...
    assertThat(salesOfferRepository.count()).isEqualTo(offersBefore);
    assertThat(salesOfferProductRepository.count()).isEqualTo(productsBefore);
                    ...
                    mockMvc
                        .perform(
                            post("/api/v1/sales/invoices")
                                ...
                                .content(
                                    """
                                    {"customerId":null,"doctorId":null,"prescriptionReference":null,"prescriptionVerified":false,"idempotencyKey":"offer-roll-inv","lines":[{"productId":"%s","batchId":"%s","quantity":1,"unit":"Tablet","mrpPaise":12000,"sellingPricePaise":10000,"discountPaise":0}]}
                                    """
