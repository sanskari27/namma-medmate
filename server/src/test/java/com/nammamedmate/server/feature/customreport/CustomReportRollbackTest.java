...
class CustomReportRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void ac05_previewAndExportDoNotMutateSaleOrStockFacts() throws Exception {
    Cookie cookie = loginOwner("cr-roll");
    UUID branchId = locationRepository.findAll().get(0).getId();
    selectBranch(cookie, branchId);
    Stocked product = stocked(cookie, "CR-ROLL", "Roll Pack");
    UUID invoiceId = createDraft(cookie, product, "cr-roll-d");
    completeCash(cookie, invoiceId, 1, "cr-roll-c");
    ...
    UUID tenantId = tenantRepository.findAll().get(0).getId();
    long auditsBefore =
        auditEventRepository
            .findByTenantIdAndCreatedAtGreaterThanEqualOrderByCreatedAtDesc(
                tenantId, Instant.now().minusSeconds(120))
            .stream()
            .filter(event -> CustomReportPolicy.ACTION.equals(event.getAction()))
            .count();

    String today = LocalDate.now(IST).toString();
    String body =
        """
        {"dataset":"SALES","columns":["invoiceNumber","productName"],"filters":[],"from":"%s","to":"%s"}
        """
