...
class PurchaseOrderRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void invalidCreateLeavesNoPartialPurchaseOrder() throws Exception {
    Tenant tenant = persistTenant("roll-po", "Roll PO");
    persistPlan(tenant.getId(), PlanCode.FREE);
    persistUser(tenant.getId(), "owner@roll-po.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId());
    Cookie cookie = login("owner@roll-po.local");
    mockMvc
        .perform(
    ...
    assertThat(purchaseOrderLineRepository.count()).isZero();
    assertThat(purchaseOrderVersionRepository.count()).isZero();
            ...
            post("/api/v1/purchase-orders")
                ...
                .content(
                    """
                    {
                      ...
                      "paymentTerms":"CREDIT",
                      "idempotencyKey":"roll-po",
                      "lines":[{"productId":"%s","quantity":1,"unitRatePaise":100}]
                    }
                    """
