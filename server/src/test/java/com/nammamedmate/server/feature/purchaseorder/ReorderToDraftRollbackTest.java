...
class ReorderToDraftRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void invalidFromReorderLeavesNoPartialDrafts() throws Exception {
    Tenant tenant = persistTenant("roll-re", "Roll reorder");
    persistPlan(tenant.getId(), PlanCode.GROWTH);
    persistUser(tenant.getId(), "owner@roll-re.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId());
    Cookie cookie = login("owner@roll-re.local");
    mockMvc
        .perform(
    ...
    assertThat(reorderRunRepository.count()).isZero();
  }
