...
class SalesInvoiceHoldRollbackTest extends AbstractIntegrationTest {

  private static final String PASSWORD = "counter-pass-1";
  private static final Instant T0 = Instant.parse("2026-09-05T08:30:00Z");
  ...
  @Test
  void ac05_failedCompleteAfterHoldLeavesNoStockCreditOrHistory() throws Exception {
    Tenant tenant = persistTenant("hold-roll", "Hold Roll");
    persistPlan(tenant.getId());
    persistUser(tenant.getId(), "owner@hold-roll.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId());
    Cookie cookie = login("owner@hold-roll.local");
    mockMvc
        .perform(
    ...
    mockMvc
        .perform(
            post("/api/v1/inventory/receipts")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"productId\":\"%s\",\"batchNumber\":\"LOT-HR\",\"manufacturedOn\":\"2026-01-15\",\"expiresOn\":\"2027-06-30\",\"purchasePricePaise\":12500,\"quantity\":\"10\",\"idempotencyKey\":\"hold-roll-recv\",\"expectedVersion\":0}"
                        .formatted(productId)))
        .andExpect(status().isOk());
    UUID batchId =
        UUID.fromString(
                    ...
                    mockMvc
                        .perform(
                            post("/api/v1/customers")
                                .cookie(cookie)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"name\":\"Hold Roll\",\"phone\":\"9511000001\"}"))
                        .andExpect(status().isOk())
                        .andReturn()
                        .getResponse()
                        .getContentAsString())
                    ...
                    mockMvc
                        .perform(
                            post("/api/v1/sales/invoices")
                                ...
                                .content(
                                    """
                                    {"customerId":"%s","doctorId":null,"prescriptionReference":"RX-ROLL","prescriptionVerified":true,"idempotencyKey":"hold-roll-inv","lines":[{"productId":"%s","batchId":"%s","quantity":1,"unit":"Tablet","mrpPaise":12000,"sellingPricePaise":10000,"discountPaise":0}]}
                                    """
    ...
    BigDecimal qtyBefore =
        stockBalanceRepository
            .findByTenantIdAndBranchIdAndProductIdAndBatchId(
                tenant.getId(), branch.getId(), productId, batchId)
            .orElseThrow()
            .getQuantity();
    long historyBefore = historyFactRepository.count();
    ...
    mockMvc
        .perform(
            post("/api/v1/sales/invoices/" + invoiceId + "/complete")
                ...
                .content(
                    """
                    {"expectedVersion":2,"expectedTotalPaise":11200,"changePaise":0,"idempotencyKey":"hold-roll-complete","payments":[{"mode":"CREDIT","amountPaise":11200}]}
                    """))
    ...
    assertThat(invoice.getStatus()).isEqualTo(SalesInvoiceStatus.HELD);
    ...
    assertThat(
            stockMovementRepository.findAllByTenantIdAndBranchIdOrderByOccurredAtDesc(
                tenant.getId(), branch.getId()))
        .noneMatch(row -> row.getType() == StockMovementType.STOCK_OUT);
    assertThat(historyFactRepository.count()).isEqualTo(historyBefore);
