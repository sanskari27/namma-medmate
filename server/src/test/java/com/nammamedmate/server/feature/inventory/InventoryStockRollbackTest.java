...
@ActiveProfiles("test")
@Testcontainers
class InventoryStockRollbackTest {
  ...
  static final PostgreSQLContainer<?> POSTGRES =
      new PostgreSQLContainer<>("postgres:16-alpine")
          .withDatabaseName("nammamedmate_inventory_stock_rollback")
  ...
  @Test
  void insufficientIssueLeavesBalanceAndMovementsUnchanged() throws Exception {
    Tenant tenant = persistTenant("inv-roll", "Inv Roll");
    persistPlan(tenant.getId(), PlanCode.FREE);
    AppUser owner = persistUser(tenant.getId(), "owner@inv-roll.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId(), "Main", "BR01", true);
    Cookie cookie = login("owner@inv-roll.local");
    selectBranch(cookie, branch.getId());
    UUID productId = createBatchedProduct(cookie);
    ...
    mockMvc
        .perform(
            post("/api/v1/inventory/receipts")
                ...
                .content(
                    """
                    {"productId":"%s","batchNumber":"LOT-R","manufacturedOn":"2026-01-01","expiresOn":"2027-01-01","purchasePricePaise":1000,"quantity":5,"idempotencyKey":"roll-in","expectedVersion":0}
                    """
                        .formatted(productId)))
    ...
    try {
      inventoryStockService.issue(
          principal, productId, batchId, new BigDecimal("6"), "roll-out", 1L);
    ...
    assertThat(stockBalanceRepository.findAll().get(0).getVersion()).isEqualTo(1L);
  }

  @Test
  void conflictingBatchIdentityLeavesNoPartialWrite() throws Exception {
    Tenant tenant = persistTenant("inv-roll2", "Inv Roll2");
    persistPlan(tenant.getId(), PlanCode.FREE);
    AppUser owner =
        persistUser(tenant.getId(), "owner@inv-roll2.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId(), "Main", "BR01", true);
    Cookie cookie = login("owner@inv-roll2.local");
    selectBranch(cookie, branch.getId());
    UUID productId = createBatchedProduct(cookie);
    ...
    mockMvc
        .perform(
            post("/api/v1/inventory/receipts")
                ...
                .content(
                    """
                    {"productId":"%s","batchNumber":"LOT-X","manufacturedOn":"2026-01-01","expiresOn":"2027-01-01","purchasePricePaise":1000,"quantity":2,"idempotencyKey":"roll2-in","expectedVersion":0}
                    """
                        .formatted(productId)))
        .andExpect(status().isOk());
    ...
    try {
      ...
      assertThat(ex.getCode()).isEqualTo("BATCH_IDENTITY_CONFLICT");
    }
  ...
  private UUID createBatchedProduct(Cookie cookie) throws Exception {
    UUID categoryId =
        UUID.fromString(
            objectMapper
                .readTree(
