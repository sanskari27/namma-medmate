...
class KioskRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void failedOpenAndTicketLeaveNoPartialWrite() throws Exception {
    Tenant tenant = persistTenant("roll-kiosk", "Roll Kiosk");
    persistPlan(tenant.getId(), PlanCode.FREE);
    persistUser(tenant.getId(), "owner@roll-kiosk.local", AppUserRole.pharmacy_owner);
    Location stall = persistBranch(tenant.getId(), "Kiosk", "BR01", true, BranchType.KIOSK);
    Cookie cookie = login("owner@roll-kiosk.local");
    mockMvc
        .perform(
            post("/api/v1/session/branch")
                .cookie(cookie)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"branchId\":\"" + stall.getId() + "\"}"))
        .andExpect(status().isOk());

    mockMvc
        .perform(post("/api/v1/kiosk/open").cookie(cookie))
        .andExpect(status().isUnprocessableEntity());
    assertThat(kioskSessionRepository.count()).isZero();
    ...
    mockMvc
        .perform(
            post("/api/v1/kiosk/tickets")
                .cookie(cookie)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"pickupRequest\":\"Crocin\"}"))
        .andExpect(status().isUnprocessableEntity());
    assertThat(kioskTicketRepository.count()).isZero();
  }
