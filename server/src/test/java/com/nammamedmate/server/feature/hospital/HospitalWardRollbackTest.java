...
class HospitalWardRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void invalidWardCreateLeavesNoPartialRows() throws Exception {
    Tenant tenant = persistTenant("roll-ward", "Roll Ward");
    persistPlan(tenant.getId(), PlanCode.PRO);
    persistUser(tenant.getId(), "owner@roll-ward.local", AppUserRole.pharmacy_owner);
    Location branch = persistBranch(tenant.getId(), "Main", "RW01");
    Cookie cookie = login("owner@roll-ward.local");
    ...
    assertThat(hospitalWardRepository.count()).isZero();
    assertThat(hospitalBedRepository.count()).isZero();
  }
