...
class NotificationRoutingTest extends AbstractIntegrationTest {
  ...
  private Location persistBranch(UUID tenantId, String name) {
    ...
    location.setBranchCode("BR" + Integer.toHexString(name.hashCode()));
    location.setAddressLine("1 Road");
    location.setCity("Bengaluru");
    location.setState("KA");
    location.setPincode("560001");
    location.setContactPhone("9876543210");
    location.setDrugLicenseNumber("DL-" + name.replace(' ', '-'));
    location.setOperatingHours(java.util.Map.of());
    location.setBranchType(com.nammamedmate.server.domain.BranchType.RETAIL);
    location.setStatus(com.nammamedmate.server.domain.BranchStatus.ACTIVE);
    location.setOpeningDate(java.time.LocalDate.of(2026, 1, 1));
    location.setDefaultBranch(false);
    location.setLinkedWarehouse(false);
    location.setPricingSettings(java.util.Map.of());
    location.setTaxSettings(java.util.Map.of());
