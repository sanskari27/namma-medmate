...
class AnalyticsRollbackTest extends AbstractIntegrationTest {
  ...
  @Test
  void ac05_readDoesNotMutateSaleOrStockFacts() throws Exception {
    Cookie cookie = loginOwner("an-roll");
    UUID branchId = locationRepository.findAll().get(0).getId();
    selectBranch(cookie, branchId);
    Stocked product = stocked(cookie, "AN-ROLL", "Roll Pack");
    UUID invoiceId = createDraft(cookie, product, "an-roll-d");
    completeCash(cookie, invoiceId, 1, "an-roll-c");
    ...
    mockMvc.perform(get("/api/v1/analytics").cookie(cookie)).andExpect(status().isOk());
    mockMvc.perform(get("/api/v1/analytics").cookie(cookie)).andExpect(status().isOk());
